...
const annualRate = ref(3);
const duration = ref(10);
const isAdvanced = ref(false);
const dynamicIncrease = ref(0);
const tax = ref(25);
const depositType = ref(DepositType.Monthly);

const disableTax = ref(false);
const taxIsDisabled = computed(() => !isAdvanced.value || disableTax.value);

const taxEffective = computed(() =>
  taxIsDisabled.value ? 0 : Number(tax.value),
);
const dynamicIncreaseEffective = computed(() =>
  isAdvanced.value ? Number(dynamicIncrease.value) : 0,
);

function handleSubmit() {
  emit("submit", {
    capital: Number(capital.value),
    monthlyAmount: Number(monthly.value),
  ...
  <div class="shadow-custom rounded-xl py-6 max-w-md">
    ...
    <form @submit.prevent="handleSubmit">
      <fieldset class="fieldset w-full p-6">
        ...
        <CustomInput
          v-model="annualRate"
          :label="`${t('input.expectedAnnualReturn')} (%)`"
          :info="t('info.expectedAnnualReturn')"
        />
        <NumberInput
          v-model="duration"
          :label="t('input.durationYears')"
        ...
        <div class="mb-2 flex items-center gap-2">
          ...
          <label
            ...
            :class="{ 'opacity-35': taxIsDisabled }"
          >
        ...
        <button
          type="submit"
          class="w-full btn btn-primary text-white font-semibold text-lg"
        >
          {{ t("calculate") }}
        </button>
      </fieldset>
